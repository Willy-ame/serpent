...
class Program
{
    class Pion
    {
        ...
        public void Avance(int taille)
        {
            PositionY=PositionY+LancerDe();
            if(taille <= PositionY){
                PositionX++;
    ...
    static int LancerDe()
    {
        ...
        animationdes(resultat);

        return resultat;
    }

    static void animationdes (int aleatoire)
    {
    ...
    for(int i = 0; i < 40; i++)
    {
        ...
        Console.Clear();
        Console.WriteLine("Tirage : [ "+aleatoire+" ]");
    ...
    static void Main()
    {
        ...
        // Placer le pion dans le tableau à la position (0, 0)X y
        ...
        Console.WriteLine("Lancer le dée");
        String valeur = Console.ReadLine();
        int anciennePositionY = pion.PositionY;
        int anciennePositionX = pion.PositionX;
        pion.Avance(tableau.GetLength(0));
        Console.WriteLine("XXX "+pion.PositionX);
        Console.WriteLine("YYY "+pion.PositionY);

        int nouvellePositionY = pion.PositionY;
        int nouvellePositionX = pion.PositionX;

        // Restaurer la valeur de la case précédente
        tableau[anciennePositionX, anciennePositionY] = (anciennePositionY + 1).ToString();
     
        // Mettre à jour la position du pion dans le tableau
        tableau[nouvellePositionX, nouvellePositionY] = pion.Symbole;
